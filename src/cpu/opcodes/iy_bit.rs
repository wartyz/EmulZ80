use crate::ops::{Local8, Op, R16, R8};
...
pub fn analiza_iy_bit(_: u8, n2: u8, n3: u8) -> (Op, usize) {
    let d = n2 as i8;
    let loc = Local8::Indexado8(R16::IY, d);
    let op_grp = n3 >> 6;
    let op_sub = (n3 >> 3) & 0b111;
    ...
    match op_grp {
        ...
        0b00 => {
            ...
            match reg_bits {
                ...
                _ => {
                    ...
                    (Op::LD8BIG(Local8::Reg8(reg), loc), 4)
                }
            }
        }
        ...
        0b10 => (Op::RESBIG(op_sub, loc), 4),
        0b11 => (Op::SETBIG(op_sub, loc), 4),
