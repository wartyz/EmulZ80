...
// Ejemplos:
// let bytes1 = op4!(0x10);              devuelve:  [0x10, 0x00, 0x00, 0x00]
// let bytes2 = op4!(0x20, 0x30);        devuelve:  [0x20, 0x30, 0x00, 0x00]
// let bytes3 = op4!(0x40, 0x50, 0x60);  devuelve:  [0x40, 0x50, 0x60, 0x00]
// macro_rules! op4 {
//     // Patrón 1: Un byte
//     ($o1 : expr) => {
//         [$o1, 0x00, 0x00, 0x00]
...
//     };
// }
// // Macro cuyo objetivo es verificar que, dado una serie de bytes que representan un código de operación (opcode),
// // usa la función opcode (ubicada en cpu::opcodes) devuelve el opcode esperado
// // y el número correcto de bytes que ocupa el opcode.
// macro_rules! assert_opcode {
//     // Patrón 1: Un byte de opcode. Ejemplo: assert_opcode!(NOP, 1, 0x00);
//     ($opc : expr, $bytes : expr, $o1 : expr) => {
//         // Usa 'op4!' para crear un array de 4 bytes con 'o1' al principio
//         let (opc, bytes) = $crate::cpu::opcodes::opcode(op4!($o1));
//
//         // Verifica el opcode y el número de bytes
//         assert_eq!($opc, opc, "Opcode {:?} ({:02x})", $opc, $o1);
//         assert_eq!($bytes, bytes, "Opcode {:?} ({:02x})", $opc, $o1);
//     };
//
//     // Patrón 2: Dos bytes de opcode. Ejemplo: assert_opcode!(NEG, 2, 0xED, 0x44);
//     ($opc : expr, $bytes : expr, $o1 : expr, $o2 : expr) => {
//         // Usa 'op4!' para crear un array con 'o1' y 'o2'
...
//     };
//
//     // Patrón 3: Tres bytes de opcode
//     ($opc : expr, $bytes : expr, $o1 : expr, $o2 : expr, $o3 : expr) => {
//         // Usa 'op4!' para crear un array con 'o1', 'o2' y 'o3'
...
//     };
//
//     // Patrón 4: Cuatro bytes de opcode (sin 'op4!')
//     ($opc : expr, $bytes : expr, $o1 : expr, $o2 : expr, $o3 : expr, $o4 : expr) => {
//         // Usa directamente un array de 4 bytes
...
/*#[test]
fn test_is_borrow() {
    // Caso sin préstamo (bits 0-3 iguales)
    assert!(!Z80::is_borrow(0x0F, 0x1F, 3)); // 15 vs 15 → false

    // Caso con préstamo (14 < 31 → true)
    assert!(Z80::is_borrow(0x0E, 0x1F, 3)); // 14 < 31 → true

    // Borrow en bit 6 (Carry entre bytes)
    //assert!(Z80::is_borrow(0x7F, 0xFF, 6));  // 127 < 255 → true
}*/
...
fn nop() {
    ...
    ejecutar(&mut z80, 0, 0, 0, 0, &mut c);

    // Comprueba que el PC ha cambiado
    assert_eq!(z80.reg.get_pc(), 0x01);
}

#[test]
fn ld_bc_nn() {
    // 0x01 C se define N reset, H reset
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);

    ejecutar(&mut z80, 0x01, 0xFE, 0xA8, 0, &mut c);

    assert_eq!(get_bc_test_big(&mut z80, &mut c), 0xA8FE);
}
#[test]
fn ld_0bc0_a() {
    // 0x02
    ...
    c.reg.a = 0xF3;
    // pongo en BC=0x10EF
    // z80.registros.b = 0xEF;
    // z80.registros.c = 0x10;
    //
    // c.reg.b = 0xEF;
    // c.reg.c = 0x10;
    set_bc_test_big(&mut z80, &mut c, 0xEF10);

    ejecutar(&mut z80, 0x02, 0, 0, 0, &mut c);
...
fn inc_bc() {
    ...
    ejecutar(&mut z80, 0x03, 0, 0, 0, &mut c);

    // compruebo que ahora bc=0xEF11
    //assert_eq!(z80.reg.b, 0xEF);
    //assert_eq!(z80.reg.c, 0x11);
    assert_eq!(get_bc_test_big(&mut z80, &mut c), 0xEF11);
}

#[test]
fn inc_b() {

    // 0x04 N reset, P/V detecta overflow, H,Z,S se define
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
    ...
    ejecutar(&mut z80, 0x04, 0, 0, 0, &mut c);
...
#[test]
fn dec_b() {
    // 0x05 N set, P/V detecta overflow, H,Z,S se define
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
...
fn ld_b_n() {
    ...
    ejecutar(&mut z80, 0x06, 0x5A, 0, 0, &mut c);
...
#[test]
fn rlca() {
    // 0x07 C se define N reset, H reset
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
    ...
    ejecutar(&mut z80, 0x07, 0, 0, 0, &mut c);

    // compruebo que ahora a = 0x00
    assert_eq!(z80.reg.a, 0x01);
...
fn ex_af_afp() {
    ...
    set_af_test_big(&mut z80, &mut c, 0x3412);
    ...
    ejecutar(&mut z80, 0x08, 0, 0, 0, &mut c);

    assert_eq!(z80.reg.a, 0xEF);
    assert_eq!(z80.reg.f, 0xAB);

    assert_eq!(z80.reg.ap, 0x34);
    assert_eq!(z80.reg.fp, 0x12);
}

#[test]
fn add_hl_bc() {
    // 0x09  BC -> HL     Carry se define, N=0, PV no afectado,  H se define, Z, S no afectados,
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
    ...
    ejecutar(&mut z80, 0x09, 0, 0, 0, &mut c);

    //assert_eq!(z80.reg.h, 0xE9);
    //assert_eq!(z80.reg.l, 0xB6);
    assert_eq!(get_hl_test_big(&mut z80, &mut c), 0xE9B6);
...
fn ld_a_0bc0() {
    ...
    ejecutar(&mut z80, 0x0A, 0, 0, 0, &mut c);
...
fn dec_bc() {
    ...
    ejecutar(&mut z80, 0x0B, 0, 0, 0, &mut c);

    //assert_eq!(z80.reg.b, 0xEF);
    //assert_eq!(z80.reg.c, 0x0F);
    assert_eq!(get_bc_test_big(&mut z80, &mut c), 0xEF0F);
}

#[test]
fn inc_c() {
    // 0x0C N reset, P/V detecta overflow, H,Z,S se define
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
    ...
    ejecutar(&mut z80, 0x0C, 0, 0, 0, &mut c);
...
#[test]
fn dec_c() {
    // 0x0D N set, P/V detecta overflow, H,Z,S se define
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
    ...
    ejecutar(&mut z80, 0x0D, 0, 0, 0, &mut c);

    // compruebo que ahora c = 0xFF
    assert_eq!(z80.reg.c, 0xFF);
...
fn ld_c_n() {
    ...
    ejecutar(&mut z80, 0x0E, 0x5A, 0, 0, &mut c);
...
#[test]
fn rrca() {
    // 0x0F C se define N reset, H reset
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
    ...
    ejecutar(&mut z80, 0x0F, 0, 0, 0, &mut c);
...
#[test]
fn djnz_d() {
    // 0x10 Instrucción DJNZ d  seguida del desplazamiento d (0xFE para -2) No afecta flags
    let mut z80 = Z80::default();
    let mut c = CPU::new(0xFFFF);
